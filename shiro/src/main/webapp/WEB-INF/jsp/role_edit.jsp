...
<section class="rt_wrap content mCustomScrollbar">
 <div class="crumb"><span class="dq"><img src="${ctxPath }/images/home.png" style="width:20px;margin-top:-2px;"></span><span class="dq">编辑角色</span></div>
      ...
      <form method="post">
	      <ul class="ulColumn2">
	       <li>
	        ...
	        <input type="text" class="textbox textbox_295" name="name" id="name" placeholder="输入角色标识名" value="${role.name }"/>
	       </li>
	       <li>
	        <span class="item_name" style="width:120px;">角色描述：</span>
	        <input type="text" class="textbox textbox_295" name="description" id="description" placeholder="输入角色描述" value="${role.description }"/>
	       </li>
	       <li>
		        	...
		        	<li>
			        	 <table class="table perm">
				        	...
				        	<s:iterator value="#entry.value" id="p">
				        		<tr><td><input type="checkbox" name="permId" value="${p.id }" pkey="<s:property value="key"/>"
				        					<s:iterator value="#request.permList" id="pe">
				        						<s:if test="#p.id == #pe.id">checked</s:if>
				        					</s:iterator>
				        				> ${p.name }</td></tr>
				        	</s:iterator>
...
$(document).ready(function() {
  $("#submit").click(function(e){
	  ...
	  var formData = {'id':$('#id').val(), 'name':$('#name').val(), 'description':$('#description').val(), 'permIds':permIds};
	  $.post("${ctxPath }/role_update", formData, function(data){
